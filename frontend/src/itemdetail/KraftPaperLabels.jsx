...
const KraftPaperLabels = ({ addToCart }) => {
  // Define price mapping for each material
  const priceMapping = {
    "Standard Kraft Paper": 120,
    "Premium Kraft Paper": 150,
    "Recycled Kraft Paper": 130,
  };

  // Most popular material (default selection)
  const defaultMaterial = "Standard Kraft Paper";

  const [selectedMaterial, setSelectedMaterial] = useState(defaultMaterial);
  const [mainImage, setMainImage] = useState(kraftPaperLabelsImg); // State for main image
  const [snackbarOpen, setSnackbarOpen] = useState(false); // State for Snackbar

  const kraftPaperLabelsDetails = {
    name: "Kraft Paper Labels",
    image: kraftPaperLabelsImg,
    description:
      "Our Kraft Paper Labels are perfect for adding a rustic and eco-friendly touch to your branding. These labels are durable and ideal for both indoor and outdoor use.",
    features: [
    ...
    materials: ["Standard Kraft Paper", "Premium Kraft Paper", "Recycled Kraft Paper"],
    extraImages: [kraftPaperLabelsImg2, kraftPaperLabelsImg3, kraftPaperLabelsImg4, kraftPaperLabelsImg5], // Extra images
  };
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            ...
            <Zoom>
              <img
                src={mainImage}
                alt={kraftPaperLabelsDetails.name}
                style={{
                  width: "100%",
                  borderRadius: "10px",
                  cursor: "zoom-in",
            ...
            >
              {kraftPaperLabelsDetails.extraImages.map((image, index) => (
                <Paper
                  key={index}
                ...
                >
                  <img
                    src={image}
                    alt={`Kraft Paper Labels ${index + 1}`}
                    style={{
                      width: "100px",
                      height: "100px",
                      borderRadius: "6px",
        ...
        <Grid item xs={12} md={6}>
          ...
          <ul style={{ marginLeft: "20px", marginBottom: "20px" }}>
            {kraftPaperLabelsDetails.features.map((feature, index) => (
              <li key={index}>
          ...
          >
            {kraftPaperLabelsDetails.materials.map((material, index) => (
              <Paper
                key={index}
...
export default KraftPaperLabels;